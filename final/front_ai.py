...
class FrontEnd:
    """Threading-enabled frontend for HoloLens2 data processing with rerun recording.
    ...
    def __init__(
        ...
        self.rcs = None
        self.combined_point = None
        self.world_to_pv_image = None

    def init_connection(self):
        """Initialize HoloLens connection, calibration, and streams."""
        # Start PV Subsystem
        ...
        self.calibration_lt = hl2ss_3dcv.get_calibration_rm(
            self.calibration_path, self.host, hl2ss.StreamPort.RM_DEPTH_LONGTHROW
        )
        ...
        # Start PV, RM Depth Long Throw, and EET streams
        self.sink_pv = hl2ss_mp.stream(hl2ss_lnm.rx_pv(
            self.host, hl2ss.StreamPort.PERSONAL_VIDEO, 
            width=self.pv_width, height=self.pv_height, 
            framerate=self.pv_fps, decoded_format='bgr24'
        ))
        self.sink_lt = hl2ss_mp.stream(hl2ss_lnm.rx_rm_depth_longthrow(
            self.host, hl2ss.StreamPort.RM_DEPTH_LONGTHROW
        ))
        self.sink_eet = hl2ss_mp.stream(hl2ss_lnm.rx_eet(
            self.host, hl2ss.StreamPort.EXTENDED_EYE_TRACKER, fps=self.eet_fps
        ))
    ...
    def get_data(self):
        ...
        return data_pv, data_lt, data_eet, data_pv.timestamp, data_pv.pose

    def depth_projection(self, data_pv, data_lt):
        """Generate depth map for PV image using optimized projection."""
        # Preprocess frames
        ...
        self.pv_intrinsics = hl2ss_3dcv.pv_update_intrinsics(
        ...
        return color, pv_z, color_intrinsics, color_extrinsics, depth

    def eet_projection(self, data_pv, data_lt, data_eet, color_intrinsics, color_extrinsics, depth):
        """Perform eye tracking raycasting and projection."""
        eet = data_eet.payload

        # EET raycasting
        if (hl2ss.is_valid_pose(data_pv.pose)):
            self.world_to_pv_image = hl2ss_3dcv.world_to_reference(data_pv.pose) @ hl2ss_3dcv.rignode_to_camera(color_extrinsics) @ hl2ss_3dcv.camera_to_image(color_intrinsics)
        ...
        if ((data_lt is not None) and (hl2ss.is_valid_pose(data_lt.pose)) and (data_lt.timestamp != self.last_lt_ts)):
            ...
            self.rcs = create_raycast_scene_optimized(depth, world_points)
        ...
        if ((self.world_to_pv_image is not None) and (eet is not None) and (data_eet.pose is not None) and (self.rcs is not None)):
            if (eet.combined_ray_valid):
                ...
                d = self.rcs.cast_rays(combined_ray)['t_hit'].numpy()
                if (np.isfinite(d)):
                    combined_point = hl2ss_3dcv.si_ray_to_point(combined_ray, d)
                    combined_image_point = hl2ss_3dcv.project(combined_point, self.world_to_pv_image)

        self.combined_point = combined_point
        return combined_point, combined_image_point, combined_ray, d

    def run(self):
        """Main processing loop with FPS counting and rerun recording."""
        while True:
            # Get synchronized data
            data_pv, data_lt, data_eet, timestamp, pose = self.get_data()
            if data_pv is None:
                continue

            # Process depth projection
            color, pv_z, color_intrinsics, color_extrinsics, depth = self.depth_projection(data_pv, data_lt)
            
            # Process EET projection
            combined_point, combined_image_point, combined_ray, d = self.eet_projection(
                data_pv, data_lt, data_eet, color_intrinsics, color_extrinsics, depth
            )

            # Push data to stack for downstream processing
            self.stack.push((color, pv_z, timestamp, pose))
            ...
            pv_timestamp = convert_qpc_to_datetime64(timestamp, self.utc_offset)
            rr.set_time("frame", timestamp=pv_timestamp)

            # Log camera pinhole and pose
            rr.log("/world/camera", 
                    rr.Pinhole(
...
if __name__ == "__main__":
    ...
    # Create shared stack for data passing
    ...
    try:
        # Consumer loop (downstream processing)
        while True:
            try:
                color, pv_z, timestamp, pose = frame_stack.peek(timeout=1.0)
                print(f"timestamp: {timestamp}\npose shape: {pose.shape}")

                # Display images (optional)
                cv2.imshow('RGB', color)
                cv2.imshow('Aligned Depth', hl2ss_3dcv.rm_depth_colormap(pv_z, 3.0))
